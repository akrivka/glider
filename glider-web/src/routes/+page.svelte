<script lang="ts">
	...
	function handleSegmentHover(event: MouseEvent, segment: ProcessedListeningSegment, day: Date) {
	...
	function getHeartrateForDay(
		samples: ProcessedHeartrateSample[],
		day: Date
	): ProcessedHeartrateSample[] {
		const dayStart = new Date(day);
		dayStart.setHours(0, 0, 0, 0);
	...
	function getHeartrateYPosition(timestamp: Date, day: Date): number {
		...
		const clampedMinutes = Math.max(
			0,
			Math.min((DAY_END_HOUR - DAY_START_HOUR) * 60, minutesFromStart)
		);
		...
		<div class="flex flex-shrink-0">
			...
			<div class="flex flex-1 overflow-x-auto">
				{#each weekDays as day}
					<div
						class="flex min-w-[120px] flex-1 flex-col border-r border-slate-800/50 last:border-r-0"
					>
						<!-- All-day events row -->
						{#if allDayEvents.length > 0}
					...
					<div class="flex min-w-[140px] flex-1 border-r border-slate-800/50 last:border-r-0">
						...
						<div class="relative flex-1" style="height: {HOURS.length * HOUR_HEIGHT}px">
							...
							{#each HOURS as hour, hourIndex}
								<div
									class="absolute right-0 left-0 border-t border-slate-800/50 {hourIndex === 0
										? 'border-transparent'
										: ''}"
						...
						>
							<!-- Spotify logo at top -->
							<div class="absolute top-0 right-0 left-0 flex justify-center pt-1">
							...
							{#each getListeningForDay(listeningSegments, day.date) as segment}
								<div
									class="absolute right-0.5 left-0.5 cursor-pointer rounded-sm border-l-2 border-green-500/60 bg-green-600/40 transition-all hover:border-green-400 hover:bg-green-600/60"
								...
								{#if dayHeartrate.length > 0}
									...
									>
										...
										{#each dayHeartrate as sample, i}
											...
											{@const nextY = nextSample
												? getHeartrateYPosition(nextSample.timestamp, day.date)
												: y + 2}
											{@const height = Math.max(2, nextY - y)}
											<rect x="2" {y} width="28" {height} fill={getBpmColor(sample.bpm)} />
										{/each}
									</svg>
								{/if}
							</div>
							...
							>
								...
								<div
									...
									style="background-color: {getStressColor(
										dayStress?.stress_high ?? null,
										dayStress?.recovery_high ?? null
									)};"
								></div>
							</div>
						{/if}
		...
		<div class="flex items-center gap-1.5">
			...
			<div>
				{#if hoveredStress.stressHigh !== null || hoveredStress.recoveryHigh !== null}
					...
					{/if}
				{:else}
					<div class="text-xs text-slate-400">No stress data</div>
				{/if}
			</div>
		</div>
	</div>
...
{#if selectedSegment}
	...
	>
		...
		>
			<!-- Header -->
			<div
				class="flex items-center justify-between border-b border-slate-700 bg-slate-800/50 px-4 py-3"
			>
			...
			<div class="border-t border-slate-700 bg-slate-800/30 px-4 py-3">
				<div class="flex items-center justify-between text-xs text-slate-400">
					<span
						>{selectedSegment.tracks.length} track{selectedSegment.tracks.length !== 1
							? 's'
							: ''}</span
					>
					<span>
